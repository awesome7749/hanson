...
- `/admin` uses the existing staff password and live API. The staff detail view formats the full intake for reading. A signed eight-hour session is stored in an HTTP-only, same-site cookie (secure on the live HTTPS site), so sign-in works across Cloud Run instances. Lead mutations, photos and legacy paid lookup endpoints require staff authentication; writes also require a matching request origin. Five failed password attempts from one client IP trigger a 15-minute limit on that Cloud Run instance. The `/staff` alias no longer redirects to the login page in live mode. These controls do not make the `/admin` URL secret.
