...
As of September 13, 2026, the production build connects basic requests to the existing Hanson Google Cloud backend. The separate Sites preview remains a browser-only demonstration. Ventrix heat-pump and assessment submission is connected for new requests with sharing permission. Automated quoting, customer accounts, project-photo uploads, appointment booking and incoming Ventrix status callbacks remain deferred.
...
- `ops.hansonhome.us` serves the staff dashboard at `/` using the existing staff password and live API. The public hostname returns 404 for `/admin` and staff API paths. The staff detail view formats the full intake for reading. A signed eight-hour session is stored in an HTTP-only, same-site cookie (secure on the live HTTPS site), so sign-in works across Cloud Run instances. Lead mutations, photos and legacy paid lookup endpoints require staff authentication; writes also require a matching request origin. Five failed password attempts from one client IP trigger a 15-minute limit on that Cloud Run instance. The ops hostname remains discoverable; password authentication is still required.
...
- `VENTRIX_SUBMISSIONS_ENABLED=true` enables outgoing submission. Both newly submitted heat-pump and assessment requests with explicit `partnerConsent` are queued. The owner confirmed that both use the same partner-portal endpoint. Resumed old browser drafts must accept the updated service-sharing notice. Historical requests are not automatically backfilled; any owner-authorized recovery must retain the original consent record and audit its separate authorization.
- The local lead and PartnerDelivery entry are created in one database transaction. The outbound payload uses the saved lead ID as `external_lead_id`, preserves the ZIP as a string, joins street/unit, combines names, normalizes phone numbers, and sends date preference only for assessments when supplied. The first line of `notes` identifies the request type (Heat-pump installation / quote or Home Energy Assessment). No undocumented API discriminator is assumed. Property/utility context is included in the documented notes field because the handover does not specify property enum values.
...
- When the partner service is enabled, requests missing sharing permission receive a clear refresh-and-review error before saving. This catches forms left open before the sharing notice was deployed; they must not get a success receipt while silently skipping Ventrix. The current form restores the draft and asks for the updated permission after refresh. With partner submission disabled, basic local-only intake remains available.
